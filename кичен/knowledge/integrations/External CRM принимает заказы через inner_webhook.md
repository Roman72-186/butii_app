---
tags: [integration, External CRM, webhook, crm]
date: 2026-04-02
---
...
Inner Webhook в External CRM — это входящий эндпоинт, который принимает POST-запросы, находит контакт по указанному полю и записывает переменные в его карточку, затем запускает привязанный сценарий.
...
```
https://rb786743.CRM.ru/inner_webhook/1f829cc9-3da3-4485-a97d-350e0d34baa1
```
...
**Контакт должен существовать в External CRM до отправки заказа.**

Пользователь должен сначала написать `/start` боту → External CRM создаст контакт с `telegram_id`. Если контакта нет, External CRM не сможет найти его по `search` и заказ «потеряется».

## Как External CRM обрабатывает запрос
...
Запись переменных в карточку контакта
    ↓
Запуск привязанного сценария (настраивается в External CRM):
  — Отправить сообщение покупателю в Telegram
  — Уведомить менеджера
  — Установить тег "Новый заказ"
```

## Переменные в шаблонах External CRM
